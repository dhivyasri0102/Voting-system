/**
 * Dynamic Voter Verification and Local OTP Simulator
 * 
 * Replaces hardcoded mock voters with dynamic persistent lookups from DataStoreService.
 * Looks up voter IDs and retains a local OTP simulator for development-only tests.
 * 
 * Enforces:
 * 1. Voter ID lookup from persistent Electoral Roll (DataStoreService)
 * 2. Local development OTP simulation through SmsService
...
export class VoterVerificationService {
  ...
  /**
  * Legacy development-only OTP simulation
   */
  public static async startOtp(
  voterId: string
): Promise<{
  success: boolean;
  ...
  const lockout =
    SecurityMonitoringService.isLockedOut(cleanId);

  if (lockout.isLocked) {
    return {
      success: false,
      message:
        `Security Lockout active for this voter identifier. ` +
        `Please retry after ${lockout.remainingSeconds} seconds.`
    };
  }
  ...
  if (!voter) {
    return {
      success: false,
      message:
        `Voter ID ${cleanId} not found in electoral roll.`
    };
  }
  ...
  const existingVerId =
    this.voterOtpIndex.get(cleanId);

  if (existingVerId) {

    const existingSession =
      this.otpSessions.get(existingVerId);

    if (existingSession) {

      const elapsed =
        (Date.now() - existingSession.lastRequestedAt) /
        1000;

      if (elapsed < 30) {

        const cooldownRemaining =
          Math.ceil(30 - elapsed);
  ...
  if (DataStoreService.hasVoted(cleanId)) {
    return {
      success: false,
      message:
        'This voter identity has already cast a ballot in this election cycle.'
    };
  }

  const targetMobile = voter.mobileNumber;
  ...
  const verificationId =
    `VER-${crypto.randomBytes(8).toString('hex').toUpperCase()}`;

  const now = Date.now();

  /*
  * Store the generated OTP for local simulator verification.
   */
  /*
  * Dispatch through the local development simulator.
   */
  const smsResult =
  ...
  /*
  * Do not create a local session if the simulator is unavailable.
   */
  if (!smsResult.success) {

    console.error(
      '[OTP] SMS dispatch failed:',
      smsResult.error
    );
    ...
    return {
      ...
      message:
        `OTP could not be sent. ${smsResult.error || ''}`
    };
  }
  ...
  this.voterOtpIndex.set(
    cleanId,
    verificationId
  );

  const maskedMobile =
    targetMobile.length >= 4
      ? targetMobile
          .slice(-4)
          .padStart(targetMobile.length, '•')
      : '••••';
  ...
  return {
    ...
    provider: smsResult.provider,
    message:
      `OTP successfully sent to ${maskedMobile}.`
  };
}
  ...
  const session =
    this.otpSessions.get(verificationId);
  ...
  if (!session) {
    ...
    return {
      verified: false,
      message:
        'Session expired or invalid verification ID. Please request a new OTP.'
    };
  }
  ...
  if (session.blocked) {
    ...
    return {
      ...
      blocked: true,
      message:
        'Maximum verification attempts exceeded. Please restart verification.'
    };
  }
  ...
  if (Date.now() > session.expiresAt) {
    ...
    return {
      verified: false,
      message:
        'OTP has expired (2 minutes limit). Please request a new OTP.'
    };
  }

  /*
  * Verify against the local simulator OTP.
   */
  const verificationResult = {
    success: Boolean(session.otp) && SecurityMonitoringService.timingSafeCompare(session.otp, enteredOtp.trim()),
  };
  ...
  if (!verificationResult.success) {
    ...
    SecurityMonitoringService.recordFailedAuthAttempt(
      session.voterId
    );
  ...
  /*
  * OTP verified by the local simulator.
   */
  SecurityMonitoringService.clearFailedAuthAttempts(
    session.voterId
  );
